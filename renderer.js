...
(function () {
  ...
  function drawBentArm(ctx, shoulder, elbow, hand, uniform) {
    if (Parts.drawBentArm) {
  ...
  function drawGripHand(ctx, hand) {
    if (Parts.drawHand) Parts.drawHand(ctx, hand.x, hand.y, P.gloves, { large: true });
  }
  ...
  function renderFrame(ctx, stageW, stageH, cfg, anim, frameIdx, facing) {
    ...
    if (pack) {
      withBodyScale(ctx, originX, originY, function () {
        Parts.drawBackpack(ctx, torsoTL_x, torsoTL_y, pack);
      });
    }
    ...
    if (supportHand && !frame.tuck) {
      ...
      drawBentArm(ctx, shoulderBack, elbow, supportHand, uniform);
    }

    withBodyScale(ctx, originX, originY, function () {
      Parts.drawLegs(ctx, legsTL_x, legsTL_y, pants, stanceLegs(frame, hold));
      ...
      Parts.drawNeck(ctx, originX + upperBodyDXLocal, headTL_y + 9, torsoTL_y, skin);
    ...
    if (supportHand && showWeapon) drawGripHand(ctx, supportHand);
    ...
    if (frame.useGrenade) {
      ...
      drawBentArm(ctx, shoulderFront, elbow, throwHand, uniform);
    } else if (!frame.tuck) {
      const elbow = {
      ...
      drawBentArm(ctx, shoulderFront, elbow, weaponGrip, uniform);
    }
